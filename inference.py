...
def predict(model, spectrogram, target_std, global_cond=None, fast_sampling=True, T=None, alpha=None, alpha_cum=None, beta=None):
    with torch.no_grad():
        ...
        audio = torch.randn(spectrogram.shape[0],
                            ...
                            model.params.hop_samples * spectrogram.shape[-1] // model.params.audio_channels,
...
def main(args):
    ...
    if args.fast:
        if T_OVERRIDE is not None:
            if T_OVERRIDE == 6:
                NOISE_OVERRIDE = params.noise_schedule[0:1] + [0.0215, 0.045, 0.08, 0.2, 0.5] # interpolate new steps from PriorGrad since we use a transformation
            elif T_OVERRIDE == 3:
    ...
    for i, features in tqdm(enumerate(dataset_test)):
        ...
        with torch.no_grad():
            ...
            for n in range(len(alpha) - 1, -1, -1):
                ...
                if n > 0:
                    ...
                    audio += sigma * noise
                audio = torch.clamp(audio, -1.0, 1.0)
        audio_pred = idwt((audio[:, 0:1, :], [audio[:, 1:2, :]])).squeeze(1)
        # normalize
        ...
        torchaudio.save(os.path.join(sample_path, sample_name), audio_pred.cpu(), sample_rate=model.params.sample_rate)
